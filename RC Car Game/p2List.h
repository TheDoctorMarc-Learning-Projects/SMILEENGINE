...
*/
	...
	void InsertAfter(uint position, const p2List<tdata>& list)
	{
		...
		{
			p2List_item<tdata>* p_DBG_NEW_item = DBG_NEW p2List_item<tdata>(p_other_list->data);

			p_DBG_NEW_item->next = (p_my_list) ? p_my_list->next : NULL;

			if (p_DBG_NEW_item->next != NULL)
				p_DBG_NEW_item->next->prev = p_DBG_NEW_item;
			else
				end = p_DBG_NEW_item;

			p_DBG_NEW_item->prev = p_my_list;

			if (p_DBG_NEW_item->prev != NULL)
				p_DBG_NEW_item->prev->next = p_DBG_NEW_item;
			else
				start = p_DBG_NEW_item;

			p_my_list = p_DBG_NEW_item;
